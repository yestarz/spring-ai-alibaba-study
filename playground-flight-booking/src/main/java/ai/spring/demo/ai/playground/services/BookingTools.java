...
public class BookingTools {
	...
	public record GetPayUrlRequest(String bookingNumber, String name){}
	public record GetPayStatusRequest(String bookingNumber){}
	...
	@Bean
	@Description("获取支付链接")
	public Function<GetPayUrlRequest, String> getPayUrl() {
		return request -> {
			return flightBookingService.getPayUrl(request.bookingNumber(), request.name());
		};
	}


	@Bean
	@Description("获取支付链接，返回true则表示支付成功，false为支付失败")
	public Function<GetPayStatusRequest, Boolean> getPayStatus() {
		return request -> {
			return flightBookingService.getPayStatus(request.bookingNumber());
		};
	}
}
