...
public class FlightBookingService {
	...
	private static final Map<String, Boolean> payStatusMap = new ConcurrentHashMap<>();
	...
	public String getPayUrl(String bookingNumber,String name){
		payStatusMap.put(bookingNumber, false);

		scheduledExecutorService.schedule(() -> {
			payStatusMap.put(bookingNumber, true);
		}, 5, TimeUnit.SECONDS);
		return "https://www.paypal.com/checkoutnow?token=" + bookingNumber + "&PayerID=" + name;
	}

	public boolean getPayStatus(String bookingNumber){
		return payStatusMap.get(bookingNumber);
	}
}
